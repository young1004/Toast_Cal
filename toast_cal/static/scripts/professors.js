...
// 교수 강의 탭의 강의 개설 버튼 eventListener
...
// 교수 강의 탭의 강의 삭제 버튼 jquery eventListener
...
shareProBtn.addEventListener('click', async function(event) {
    ...
    $('#pubCalLoadBtn').trigger('click'); // 자동으로 공유 캘린더 데이터 받아옴
});
...
pubCalLoadBtn.addEventListener('click', async function() {
    ...
    uiLock('calProBtn', 'subProBtn', 'voteProBtn', 'shareProBtn', 'pubcal_select', 'pubStart', 'pubEnd', 'delete_vote', 'portal_to_correct_vote', 'delete_vote');
    ...
    if (betweenTodayStart < -1) { // 시작날짜가 오늘 이전의 날짜가 입력됬을때
        ...
        if ((betweenStartWeek / 7) < 1) {} else if ((betweenStartWeek / 7) < 2) { // 주 단위로 이동하기 위함
            pubCalendar.next();
    ...
    // 막아줬던 부분들을 풀어주는 함수 실행
    uiUnLock('calProBtn', 'subProBtn', 'voteProBtn', 'shareProBtn', 'pubcal_select', 'pubStart', 'pubEnd', 'delete_vote', 'portal_to_correct_vote', 'delete_vote');
});

// 공유 캘린더 '과목명 (과목코드)' 변경을 감지하는 eventListener
// 공유 캘린더 과목코드 변경시 즉시 right_nav위치의 투표정보를 바꿔주는 함수
$('#pubcal_select').on('change', function() {
    ...
    $('#pubCalLoadBtn').trigger('click'); // 자동으로 공유 캘린더 데이터 로드
    
});

// 공유 캘린더 시작 날짜 date 필드 eventListener
$("#pubStart").on("change", function() {
    $('#pubCalLoadBtn').trigger('click'); // 자동으로 공유 캘린더 데이터 로드
});

// 공유 캘린더 끝 날짜 date 필드 eventListener
$("#pubEnd").on("change", function() {
    $('#pubCalLoadBtn').trigger('click'); // 자동으로 공유 캘린더 데이터 로드
});

// 공유캘린더 오른쪽 사이드바 투표수정 버튼 (투표정보 있는 경우)
